    ...
    public partial class FrmODC : DevComponents.DotNetBar.Metro.MetroForm
    {
        ...
        private void btnConsultar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    var OdcInfo = context.pODCinfo(txtEmpresa.Text.Trim(), txtFact_num.Value).First();
                    if (OdcInfo != null)
                    {
        ...
        private void txtEmpresa_Leave(object sender, EventArgs e)
        {
            try
            {
                ...
                var Emp = Dt.Where(c => c.co_empresa.Trim() == txtEmpresa.Text.Trim()).First();
                if (Emp == null)
                {
        ...
        private void btnSeleccionar_Click(object sender, EventArgs e)
        {
            try
            {
                if (Lrodc.Count() > 0)
                {
                    foreach (var item in Lrodc)
                    {
                        item.sel = btnSeleccionar.Text.Trim() == "&Seleccionar todo..." ? true : false;
                    }
                    grdDatos.Refresh();
