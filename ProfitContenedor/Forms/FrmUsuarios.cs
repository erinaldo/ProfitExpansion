        ...
        private void Buscar()
        {
            try
            {
                using (Profit_RGEntities context = new Profit_RGEntities())
                {
                    ...
                    var user1 = context.Usuarios.Where(xD => xD.nombre.Trim() == txtBuscar.Text.Trim()).ToList();

                    if (user1.Count > 0)
                    {

                        CargarUsuario(user1[0].co_usuario.Trim(), context);
        ...
        private void Navegar(int p)
        {
            try
            {
                ...
                using (Profit_RGEntities context = new Profit_RGEntities())
                {
                    ...
                    if (pusu.Count > 0)
                    {
                        CargarUsuario(pusu[0].Trim(), context);
                        CargarDetalle();
                    }
                    else
                    {
