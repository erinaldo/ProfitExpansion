            ...
            try
            {
                ...
                using (Profit_RGEntities context = new Profit_RGEntities())
                {
                    ...
                    foreach (var item in Detalle)
                    {
                        ...
                        Irodc.uni_venta = item.uni_venta;
                        Lrodc.Add(Irodc); 
                    }
        ...
        private void btnImportar_Click(object sender, EventArgs e)
        {
            try
            {
                using (Profit_RGEntities context = new Profit_RGEntities())
                {
                    ...
                    {
                        if (item.sel == true)
                            context.pInsertarReng_emb(item.empresa.Trim(), Nro_viaje, item.fact_num, item.co_art.Trim(), item.art_des.Trim(), item.total_imp,item.prec_vta,item.tasa,txtProv.Text.Trim(),item.uni_venta);
                    }

                    context.pNotificacionEmailEmbarque(Nro_viaje.ToString());
