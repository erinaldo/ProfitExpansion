        ...
        private void Navegar(int p)
        {
            try
            {
                ...
                using (Profit_RGEntities context = new Profit_RGEntities())
                {
                    ...
                    if (pemb.Count>0)
                    {
                        embarque = context.pSeleccionarEmbarque(pemb[0].Value).First();
                        ...
                        txtFec_sal.Value = embarque.fecha_sal.Value;
                        txtPto_destino.Text = embarque.pto_lleg.Trim();
                        txtFec_lleg.Value = embarque.fecha_lleg.Value;
        ...
        private void txtCo_consig_Leave(object sender, EventArgs e)
        {
            try
            {
                if (txtCo_consig.Text.Trim().Length > 0)
                {
                    ...
                    var Emp = Dt.Where(c => c.co_empresa.Trim() == txtCo_consig.Text.Trim()).First();
                    if (Emp == null)
                    {
            ...
            try
            {
                    ...
                    MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) == System.Windows.Forms.DialogResult.Yes)
                {
                    using (Profit_RGEntities context = new Profit_RGEntities())
                    {
                        ...
                        {
                            System.Guid? R = new Guid(grdDatos[5, grdDatos.CurrentCell.RowIndex].Value.ToString());
        ...
        private void txtCo_shipper_Leave(object sender, EventArgs e)
        {
            try
            {
                if (txtCo_shipper.Text.Trim().Length > 0)
                {
                    ...
                    using (Profit_RGEntities context = new Profit_RGEntities())
                    {
                        shipper = context.pSeleccionarProv(txtCo_consig.Text.Trim()).Where(s => s.co_prov.Trim() == txtCo_shipper.Text.Trim()).First();
                    }
                    if (shipper == null)
                    {
        ...
        private void txtCo_naviera_Leave(object sender, EventArgs e)
        {
            try
            {
                if (txtCo_naviera.Text.Trim().Length > 0)
                {
                    ...
                    using (Profit_RGEntities context = new Profit_RGEntities())
                    {
                        naviera = context.pSeleccionarNaviera(txtCo_naviera.Text.Trim()).First();
                    }
                    if (naviera == null)
                    {
