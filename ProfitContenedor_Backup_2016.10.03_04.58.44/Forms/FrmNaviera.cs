    ...
    public partial class FrmNaviera : DevComponents.DotNetBar.Metro.MetroForm
    {
        ...
        private void Navegar(int p)
        {
            try
            {
                ...
                using (Profit_RGEntities context = new Profit_RGEntities())
                {
                    ...
                    if (pnav.Count>0)
                    {
                        naviera = context.pSeleccionarNaviera(pnav[0].Trim()).FirstOrDefault();
        ...
        private void txtCo_prov_Leave(object sender, EventArgs e)
        {
            try
            {
                ...
                using (Profit_RGEntities context = new Profit_RGEntities())
                {
                    shipper = context.pSeleccionarProv(txtEmpresa.Text.Trim(),txtCo_prov.Text.Trim()).FirstOrDefault();
                }
                if (shipper == null)
                {
        ...
        private void txtEmpresa_Leave(object sender, EventArgs e)
        {
            try
            {
                ...
                //emp.Fill(Dt);
                //var Emp = Dt.Where(c => c.cod_emp.Trim() == txtEmpresa.Text.Trim()).FirstOrDefault;
                using(Profit_RGEntities context = new Profit_RGEntities())
                {
                    var Emp = context.pSeleccionarEmpresa(txtEmpresa.Text.Trim()).FirstOrDefault();
                    if (Emp == null)
                    {
